...
import { nextRemoteFunctionId, registerRemoteFunction } from './remoteFunctionRegistry';
  ...
  __init: () => {
    ...
    return new Proxy({}, {
      get: (_, prop) => {
        ...
        throw new Error(`[Worklets] Trying to access property \`${String(prop)}\` of an object which cannot be sent to the UI runtime.`);
      },
      set: () => {
        throw new Error('[Worklets] Trying to write to an object which cannot be sent to the UI runtime.');
      }
    });
  }
};
...
function cloneNonWorkletFunction(fun) {
  ...
  const clone = WorkletsModule.createSerializableNonWorkletFunction(fun, functionId, __DEV__ ? fun.name : undefined);
  if (clone.__keepAlive) {
  ...
  const handle = cloneInitializer({
    __init: () => {
      ...
      if (constructor === undefined) {
        throw new Error(`[Worklets] Constructor for \`${typeName}\` not found.`);
      }
