...
import { isWorkletRuntime } from "../runtimeKind.js";
  ...
  const moduleFactory = makeModuleFactory(module => {
    module.exports = new Proxy({}, {
      get: function get(_target, prop) {
        globalThis.console.warn(`You tried to import '${String(prop)}' from 'react-native' module on a Worklet Runtime. Using 'react-native' module on a Worklet Runtime is not allowed.`, new Error().stack);
        return {
          get() {
            return undefined;
          }
...
function assertWorkletRuntime(functionName) {
  if (!isWorkletRuntime()) {
    throw new Error(`[Worklets] ${functionName} can be used only on Worklet Runtimes.`);
  }
}
